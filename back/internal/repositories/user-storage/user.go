...
func (r *Repository) Create(form user.CreateUserForm) (user.User, error) {
	var u user.User
	query := `insert into user(name, surname, avatar, age, gender, city, email, password)
				values (?, ?, ?, ?, ?, ?, ?, ?)`

	result, err := r.db.ExecContext(r.createContext(),
		query, form.Name, form.Surname, form.Avatar, form.Age, form.Gender, form.City, form.Email, form.Password)
...
func (r *Repository) GetById(id int) (user.User, error) {
	...
	err := row.Scan(&u.Id, &u.Name, &u.Surname, &u.Avatar, &u.Age, &u.Gender, &u.City, &u.Email, &u.Password)
